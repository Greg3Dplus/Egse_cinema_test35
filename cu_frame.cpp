...
void CU_Frame::init_CU_Frame(quint8 nb_bytes_per_pixel, quint8 cam_fw_type)
{
    ...
    if (this->cam_fw_type == CAM_FW_CO2M){
        ...
        this->setCMVHeaderSize(29*8);
    }
    else {
        this->setCMVHeaderSize(29);
    }
    qDebug().noquote()  << "nb_bytes_per_pixel " + QString::number(nb_bytes_per_pixel,10);
}
...
//      qDebug().noquote()  << "Video Packet (" + QString::number(i,10) + ") = 0x" + QString::number(quint8(packet[24*i+23]),16);


//    }
//    for (quint8 i = 0; i < 29; i++) {
//      qDebug().noquote()  << "getbyte = 0x" + QString::number(quint8(this->getByte(24*i+23)),16);
...
//      qDebug().noquote()  << "header (" + QString::number(i,10) + ") = 0x" + QString::number(static_cast<quint8>(this->getHeader_field(i)));
//    }
     ...
     qDebug().noquote()  << "width       = 0x" + QString::number(this->getWidth(),16);
     qDebug().noquote()  << "height      = 0x" + QString::number( this->getHeight(),16);
//     qDebug().noquote()  << "frame tag   = "+ this->getFrame_tag();
       qDebug().noquote()  << "exp time    = " + QString::number(this->getExp_reg(),16);
       qDebug().noquote()  << "gain        = " + QString::number(this->getGain(),16);
...
quint32 CU_Frame::getData(quint32 address, quint8 nb_bits, bool no_shift) {
    ...
    if(this->cam_fw_type == CAM_FW_CO2M || this->cam_fw_type == CAM_FW_CINEMA){
        ...
        for (quint8 byte_num = 0; byte_num < this->nb_bytes_per_pixel; byte_num++) {
            ...
            //quint8 Byte_val = this->getByte(this->nb_bytes_per_pixel*address+((nb_bytes_per_pixel-1) - byte_num));


//              qDebug().noquote()  << "Byte_val " + QString::number(address,10) + " (" + QString::number(byte_num,10) + ") = 0x" + QString::number(static_cast<quint8>(Byte_val));
...
//    if (this->cam_fw_type == CAM_FW_CO2M || this->cam_fw_type == CAM_FW_CINEMA){
        return static_cast<quint8>(this->getData(8*num_field+7, 8));
//
      //  return static_cast<quint8>(this->packet[24*num_field+23]);
//}
//    else {
...
//        return  static_cast<quint16> ((this->getHeader_field(20) << 8) | this->getHeader_field(19)) +1;
//     else
...
//    if (this->cam_fw_type == CAM_FW_CINEMA)
//        return static_cast<quint16> ((this->getHeader_field(22) << 8) | this->getHeader_field(21)) +1;
//    else
...
//    if (this->cam_fw_type == CAM_FW_CINEMA)
//        return  static_cast<quint16> ((this->getHeader_field(24) << 8) | this->getHeader_field(23));
//    else
...
//    if (this->cam_fw_type == CAM_FW_CINEMA)
//         return  static_cast<quint16> ((this->getHeader_field(26) << 8) | this->getHeader_field(25));
//    else
...
quint8 CU_Frame::getFooter_size(void) {
    if (this->cam_fw_type == CAM_FW_CINEMA)
        return this->getHeader_field(27);
    else
...
quint32 CU_Frame::compute_ref_PktSize(quint16 width, quint16 height, quint8 nb_bytes, quint16 CMV_HEADER_SIZE, quint8 footer_size) {
...
quint32 CU_Frame::getExp_reg(void) {
    ...
    quint32 dataCMV = 0;
    for (quint8 data_num = 0; data_num < 4; data_num++) {
        dataCMV |= static_cast<quint32> ((this->getHeader_field(data_start_ad + data_num)) << (8*data_num));
    }
    return dataCMV;
}

quint16 CU_Frame::getGain(void) {
    return  static_cast<quint16> ((this->getHeader_field(18)));
}
...
void CU_Frame::create_txt_file(QString file_name,  // "image_test.txt",
                               double sens_in_freq, quint16 ADC_gain, quint8 vramp1, quint8 vramp2, quint16 offset, quint8 fot_length, quint8 cam_fw_type, double H_Period)
{
    ...
    if (outputFile.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
        ...
        quint32 integration_time  = this->getExp_reg();
        quint8 footer_size        = this->getFooter_size();
        quint16 gain              = this->getGain();

        // Compute temperature according to datasheet
        double temperature = this->getTemp_val(sens_in_freq);
        ...
        double exp_time_int = (integration_time + 0.43*fot_length) * 129/sens_in_freq;
        if (cam_fw_type != CAM_FW_CINEMA){
             exp_time_int = (integration_time + 0.43*fot_length) * 129/sens_in_freq;
       }
       else  {

             exp_time_int =  (H_Period*(((this->getHeight()*binning_y)+VB1+frameInfo)-integration_time)) + 14.26; // imx_freq
        }
        out << "Integration time : "   << dec << exp_time_int  << " us" << " (register set to 0x" << hex << integration_time << ")\n";
        out << "Gain (dB)        : "   << dec << gain/10 << " (register set to 0x" << gain << ")" <<  "\n";
...
QPixmap CU_Frame::toPixMap(quint8 color) {
    // Read 2 bytes each time from the array to form a pixel

    static quint8 final_pic_cmv [4*MAX_HEIGHT_CMV*MAX_WIDTH_CMV];
    static quint8 final_pic_imx [4*MAX_HEIGHT_IMX*MAX_WIDTH_IMX];
 ...
 if (color == 0) {
    for(quint16 line = 0; line < this->getHeight(); line++) {
        for(quint16 pix = 0; pix < width; pix++) {
            ...
            quint32 pixel =  this->getPixel(line, pix, this->getNb_bits());
            if (this->cam_fw_type == CAM_FW_3DIPCC0802) {
                pixel = pixel >> (this->getNb_bits() - 8);
            }
            if (this->cam_fw_type == CAM_FW_CINEMA) {
            final_pic_imx[4*(width*line + pix)]   = static_cast<quint8>(pixel);
            final_pic_imx[4*(width*line + pix)+1] = static_cast<quint8>(pixel);
            final_pic_imx[4*(width*line + pix)+2] = static_cast<quint8>(pixel);
            final_pic_imx[4*(width*line + pix)+3] = 0xff;


            }
        else {
            final_pic_cmv[4*(width*line + pix)]   = static_cast<quint8>(pixel);
            final_pic_cmv[4*(width*line + pix)+1] = static_cast<quint8>(pixel);
            final_pic_cmv[4*(width*line + pix)+2] = static_cast<quint8>(pixel);
            final_pic_cmv[4*(width*line + pix)+3] = 0xff;
        }
      }
    }
 }else{
     for(quint16 line = 0; line < this->getHeight(); line++) {
         for(quint16 pix = 0; pix < width; pix++) {
             ...
             if (this->cam_fw_type == CAM_FW_3DIPCC0802) {
                ...
                quint16 R =  this->getPixelRGB(line, 4*pix+1, 8);
                quint16 G =  this->getPixelRGB(line, 4*pix+2, 8);
                quint16 B =  this->getPixelRGB(line, 4*pix+3, 8);
            if (this->cam_fw_type == CAM_FW_CINEMA) {
                final_pic_imx[4*(width*line + pix)]   = (unsigned char) B;
                final_pic_imx[4*(width*line + pix)+1] = (unsigned char) G;
                final_pic_imx[4*(width*line + pix)+2] = (unsigned char) R;
                final_pic_imx[4*(width*line + pix)+3] = 0xff;

             }
         else {
                final_pic_cmv[4*(width*line + pix)]   = (unsigned char) B;
                final_pic_cmv[4*(width*line + pix)+1] = (unsigned char) G;
                final_pic_cmv[4*(width*line + pix)+2] = (unsigned char) R;
                final_pic_cmv[4*(width*line + pix)+3] = 0xff;
         }
       }
     }
  }
    // qDebug().noquote() << "Before return QPixMap";

    int step = 0;

    // QImage needs an alignment with 32-bits. We cannot use Format GrayScale 8
    if (this->cam_fw_type == CAM_FW_CINEMA)
        return QPixmap::fromImage(QImage(final_pic_imx, this->getWidth(), this->getHeight(), step, QImage::Format::Format_RGB32));
    else
        return QPixmap::fromImage(QImage(final_pic_cmv, this->getWidth(), this->getHeight(), step, QImage::Format::Format_RGB32));
